...
public class ApiControllerAdvice {
    ...
    @ExceptionHandler
    public ResponseEntity<ApiResponse<?>> handleBadRequestDetailed(MethodArgumentNotValidException e) {
        BindingResult bindingResult = e.getBindingResult();
        List<FieldError> fieldErrors = bindingResult.getFieldErrors();

        if (fieldErrors.isEmpty()) {
            return failureResponse(ErrorType.BAD_REQUEST, "요청 데이터 유효성 검사에 실패했습니다.");
        }

        // 모든 필드 오류를 수집
        String allErrorMessages = fieldErrors.stream()
                .map(fieldError -> {
                    String fieldName = fieldError.getField();
                    Object rejectedValue = fieldError.getRejectedValue();
                    String defaultMessage = fieldError.getDefaultMessage();
                    String valueString = rejectedValue != null ? rejectedValue.toString() : "null";

                    return String.format("'%s': %s (입력값: %s)", fieldName, defaultMessage, valueString);
                })
                .collect(Collectors.joining(", "));

        String errorMessage = fieldErrors.size() == 1
                ? String.format("필드 유효성 검사 실패 - %s", allErrorMessages)
                : String.format("다음 %d개 필드에서 유효성 검사가 실패했습니다 - %s", fieldErrors.size(), allErrorMessages);
